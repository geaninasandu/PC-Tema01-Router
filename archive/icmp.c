...
void build_icmp_header(struct iphdr *ip_header, struct icmphdr *icmp_header, char *source_ip,
    ...
    icmp_header->checksum = ip_checksum(icmp_header, sizeof(struct icmphdr) + packet_size);
...
*destination_ip, uint8_t *destination_mac, int type, int sequence) {
    ...
    build_icmp_header(ip_reply, icmp_reply, interface_ip, destination_ip, type, sequence, sizeof
    (icmp_message->payload));
...
//    icmp_message->len = sizeof(struct ether_header) + sizeof(struct iphdr) + sizeof(struct icmphdr);
}
